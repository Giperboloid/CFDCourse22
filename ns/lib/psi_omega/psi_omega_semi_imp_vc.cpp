...
{
public:
	VelocitySolver(std::shared_ptr<AFemApproximator> appr) : _approximator(appr.get())
	{
	}
...
{
public:
	OmegaSolver(std::shared_ptr<AFemApproximator> appr) : _approximator(appr.get())
	{
	...
	{
		if (tau <= 0)
			throw std::runtime_error("Invalid tau value");
		_tau = tau;
	}
...
PsiOmega_SemiImpVC::PsiOmega_SemiImpVC(std::shared_ptr<AFemApproximator> appr) : APsiOmegaSolver(appr, appr),
